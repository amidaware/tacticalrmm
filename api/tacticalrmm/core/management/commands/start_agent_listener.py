...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            f"Running in {'foreground' if foreground else 'background'} mode"
        )
        ...
        try:
            ...
            if foreground:
                self.stdout.write(
                    self.style.SUCCESS(
                        "Listener started in foreground mode. Press Ctrl+C to exit."
                    )
                )
                self.stdout.write(
                    "Keeping main thread alive. Messages will appear below:"
                )
                try:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            "Listener stopped due to keyboard interrupt."
                        )
                    )
            else:
                self.stdout.write(
                    self.style.SUCCESS("Listener started in background mode.")
                )
                self.stdout.write(
                    "NOTE: In background mode, logs will go to the Django log file."
                )
            ...
            self.stderr.write(
                self.style.ERROR(
                    "Check Redis connection and make sure the REDIS_HOST environment variable is set correctly."
                )
            )
