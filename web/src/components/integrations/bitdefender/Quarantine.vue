<template>
  <div class="q-pa-md">
    <q-card>
      <q-table :rows="rows" :columns="columns" row-key="id" :selected-rows-label="getSelectedString"
        selection="multiple" v-model:selected="selected" :pagination="pagination">
        <template v-slot:top-left>
          <q-btn flat dense @click="getQuarantine()" icon="refresh" />
  ...
  export default {
    ...
    setup(props) {
      ...
      function getQuarantine() {
        $q.loading.show()
        axios
          .get(`/bitdefender/endpoint/quarantine/` + props.endpoint.id + `/`)
          .then(r => {
            rows.value = []
