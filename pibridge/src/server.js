...
const DEFAULT_DECISION_POLICY =
  ...
  `REPLY FORMATTING: Write customer replies (reply_to_ticket / resolve_ticket customer_html) in simple MARKDOWN - the system AUTOMATICALLY converts it to clean, branded, email-safe HTML (styled headings, bullets, numbered steps, bold, inline code, fenced code blocks). Use '## Heading', '- bullet', '1. step', '**bold**', backticks for \`commands\`, and triple-backtick fenced blocks for command output. Do NOT hand-write HTML tags/inline styles or paste raw <table>/<div> markup - just write clean, well-structured markdown (short intro, clear sections, a next-steps list when relevant) and it will look great.\n` +
  `TECHNICAL EMAIL: When the tech asks for a "technical email" / "full technical reply" / "detailed technical email", still write MARKDOWN (auto-formatted), but make it thorough: (1) a short intro with the headline conclusion; (2) a findings/specs section - use a markdown table or a bullet list of the key values; (3) fenced code blocks for command output/config; (4) an Assessment section + a prioritized next-steps list. Keep the FULL technical detail and the actual numbers. Do NOT add your own greeting/sign-off (added automatically). For any OTHER request a normal concise reply is fine.\n` +
...
function helpdeskSection(blob, clientName) {
  ...
  const fmtNote = ` Any customer-facing reply you write is AUTOMATICALLY converted from markdown to clean, branded, ` +
    `email-safe HTML - so write simple markdown (## headings, - bullets, 1. steps, **bold**, backtick ` +
    `code, triple-backtick fenced blocks), NOT raw HTML/inline styles.`;
