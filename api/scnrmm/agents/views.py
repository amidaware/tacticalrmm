...
from scnrmm.helpers import date_is_in_past, notify_error
from scnrmm.permissions import (
    _has_perm_on_agent,
    _has_perm_on_client,
    _has_perm_on_site,
)
from scnrmm.utils import get_default_timezone, reload_nats
...
class AgentProcesses(APIView):
    ...
    def get(self, request, agent_id):
        if getattr(settings, "DEMO", False):
            from scnrmm.demo_views import demo_get_procs
...
def get_event_log(request, agent_id, logtype, days):
    if getattr(settings, "DEMO", False):
        from scnrmm.demo_views import demo_get_eventlog
...
def install_agent(request):
    ...
    if request.data["installMethod"] == "exe":
        from scnrmm.utils import generate_winagent_exe
