...
if [ "${SCRIPT_VERSION}" -ne "${NEW_VER}" ]; then
    ...
    wget -q "${SCRIPT_URL}" -O install-debian.sh
    printf >&2 "${YELLOW}Script updated! Please re-run ./install-debian.sh${NC}\n"
    rm -f $TMP_FILE
    exit 1
fi
...
UBU20=$(grep 10* "/etc/debian_version")
if ! [[ $UBU20 ]]; then
  echo -ne "\033[0;31mThis script will only work on Debian 10 (Buster)\e[0m\n"
  exit 1
fi
...
echo "deb [arch=amd64] http://repo.mongodb.org/apt/debian buster/mongodb-org/4.4 main" | sudo tee /etc/apt/sources.list.d/mongodb-org-4.2.list
sudo apt update
