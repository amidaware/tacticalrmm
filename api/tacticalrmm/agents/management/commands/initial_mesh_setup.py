...
class Command(BaseCommand):
    ...
    async def websocket_call(self):
        ...
        token = get_auth_token(self.mesh_settings.mesh_username, self.mesh_settings.mesh_token.rstrip())
            ...
            async for message in websocket:
                ...
                if response["action"] == "meshes":
                    ...
                    if not response["meshes"]:
                        await websocket.send(json.dumps({"action":"createmesh","meshname":"TacticalRMM","meshtype":2,"responseid":"python"}))
                        break
                    else:
                        break
