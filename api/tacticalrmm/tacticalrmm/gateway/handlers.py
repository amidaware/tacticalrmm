...
from .audit import _audit_session_failed, _audit_exec_command, _close_session_and_audit
from .constants import ANSI_ESCAPE, TERMINAL_MODES
...
class DirectSessionHandler(asyncssh.SSHServerSession):
    ...
    def shell_requested(self):
        ...
        from .direct_terminal import DirectTerminalHandler
        return DirectTerminalHandler(
            self._user, self._agent, self._session_id, self._remote_ip,
            ...
            if b"\r" in self._term_buf or b"\n" in self._term_buf:
                ...
                for line in lines:
                    if line.strip() and not any(b < 32 for b in line if b not in (9,)):
                        from .audit import _audit_terminal_command
    ...
    def closed(self):
        if self._term:
            ...
            self._term = None
        if self._started_at:
            from .audit import _close_session_and_audit
            asyncio.create_task(
