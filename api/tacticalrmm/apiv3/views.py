...
class SupportConfig(APIView):
    ...
    def get(self, request, agentid):
        try:
            ...
            site = agent.site
            if not site:
                return Response({"error": "Site not found for the given agent."}, status=404)
            ...
            trayicon_icon_url = systray_instance.icon.url if systray_instance.icon else None
            trayicon_name = systray_instance.name

            return Response({
                "support_config": site.trayicon if site.trayicon else "Default logic for missing tray icon config",
                "systray_enabled": True,
                "support_icon": trayicon_icon_url,  # Return the URL instead of the file path
                "support_name": trayicon_name
            })
        except Exception as e:
